...
1. **Requirement-traceability matrix (source of truth).** Every normative clause worked on MUST have a definition in the Go-native `spec/` package (`spec.Matrices`): `Spec` (e.g. `draft-ietf-jmap-calendars-27`, `RFC8984`, `RFC8620`), `Section`, RFC 2119 `Level` (`spec.MUST`/`spec.MUSTNOT`/`spec.SHOULD`/`spec.SHOULDNOT`/`spec.MAY`/`spec.RECOMMENDED`/`spec.OPTIONAL`), the verbatim requirement `Text`, the `Tests` that cover it, and a `Status` of `spec.Covered`, `spec.Gap`, or `spec.NonGoal` (a gap/non-goal row carries no tests and is an explicit, reviewed acknowledgement). The `TestSpecCoverage` checker gates this: it fails on dangling test references, `covered` rows without tests, duplicate clauses, malformed levels/statuses, and out-of-order rows, and it reports every outstanding `MUST` gap. Register new matrices in `spec.Matrices`.
2. **Cite the clause in the test.** Requirement tests MUST call `spectest.Require(t, spec, section, level, text)` or `spectest.Cover(t, clause)` for each clause they exercise, so every test is self-documenting and coverage is reportable via `go test -v`.
...
5. **Always update the traceability report on code changes.** Every code change, refactoring, or new test MUST update `docs/SPEC_TRACEABILITY_REPORT.md` (and `docs/SPEC_COVERAGE.md` via `UPDATE_DOCS=1 go test ./spec`) so the mapping between RFC clauses, test functions, and production implementation files remains 100% accurate and up-to-date.
...
## IMAP/SMTP Backend (`imapsmtp`) Reference Implementation & Test Coverage Rule
The legacy in-memory backend (`jmap/memory/`) has been completely retired. `jmap/imapsmtp` is the primary and sole production-grade reference backend for all Mail (`Email`, `Mailbox`, `Thread`), Blob, Submission, Identity, VacationResponse, PushSubscription, and Quota functionality.
1. **Back Mail & Blob Features With IMAP/SMTP (`imapsmtp`)**: All mail-related features and protocol mappings MUST be implemented directly against the live IMAP/SMTP backend (`jmap/imapsmtp/`), with compile-time assertions verifying interface fulfillment (`var _ jmap.MailBackend = (*IMAPSMTPBackend)(nil)`, `var _ jmap.BlobBackend = (*IMAPSMTPBackend)(nil)`, `var _ jmap.BlobReferenceBackend = (*IMAPSMTPBackend)(nil)`).
2. **Hermetic In-Process Tests via Embedded IMAP/SMTP Backend**: Unit and integration tests MUST drive requests through `imapsmtp.NewEmbeddedBackend(testUsername)`, executing hermetically against an embedded in-process IMAP/SMTP server with 0 external daemon or Docker requirements.
3. **Production Reference Adapters for Non-Mail Domains**: Non-mail domains use embedded production reference adapters (`jmap/nextcloud` for CalDAV/CardDAV/WebDAV calendars, contacts/cards, principals, and filenodes; `jmap/managesieve` for Sieve scripts; and dedicated stores for auth and imapaccess) to allow full protocol verification and hermetic testing against real standards implementations. Mock stores (`jmap/testmock/`) have been completely retired and removed.
4. **Test the Feature, Not Just the Wiring**: Verifying that a capability is advertised and that method calls return responses with the expected names is necessary but NOT sufficient. Tests MUST prove the feature works end-to-end—for every stored object, perform a round-trip such as `set` (create) → `get`/`query` (retrieve) → assert the returned properties, then `set` (update) and `set` (destroy), asserting state changes, `notCreated`/`notUpdated`/`notDestroyed`, and change-tracking (`changes`/`queryChanges`) results. Never assert only on method names or response shape while ignoring the payload contents.
5. **IMAP/SMTP Backend Is the Reference Backend**: The `imapsmtp` backend is the canonical, complete reference implementation for mail protocols. Keep `imapsmtp` correct, standards-compliant, and fully synchronized with upstream IMAP IDLE and state changes.
...
## Guiding Principle: Always Push Back Against Custom Serializers and Parsers
**Never accept or introduce custom serializers, deserializers, string formatters, or parsers when an established, battle-tested standard library exists.** The agent MUST actively push back every time hand-rolled formatting (e.g. manual `fmt.Fprintf`/`strings.Builder` text assembly for protocols/wire formats like iCalendar, vCard, MIME, HTTP, Sieve, JSON) or ad-hoc string parsing (`strings.Split`, `strings.Index`, regex) is proposed or encountered. Hand-rolled implementations invariably suffer from subtle protocol incompatibilities, line folding bugs, escaping and injection vulnerabilities, and timezone/DST edge cases. Always use Go's standard library or canonical vendored packages (`github.com/emersion/go-ical`, `github.com/emersion/go-vcard`, `github.com/emersion/go-message`, `github.com/foxcpp/go-sieve`, `github.com/teambition/rrule-go`, etc.).

## Standard Parsers & Encoders Only — Never Ad-Hoc Parsing & Never Reinvent Libraries
...
**General Rule: Never implement yourself what an established, battle-tested library provides.**
Implementing custom parsers, line unfolders, date/time formatters, or recurrence calculators when a standard library exists drastically increases the required test matrix and introduces subtle edge cases (e.g. timezone miscalculations, DST boundaries, parser differentials, and character-escaping regressions).

1. **This is a security & reliability requirement, not a style preference.** Ad-hoc scanning is a primary source
...
2. **Use the standard library or the already-vendored library.** Always use Go's `net/mail`, `mime`,
   `mime/multipart`, `mime/quotedprintable`, `encoding/base64`, `net/url`, `time`, `encoding/json`,
   and the repository's existing `github.com/emersion/go-message`, `github.com/emersion/go-ical`,
   `github.com/emersion/go-vcard`, `github.com/teambition/rrule-go`, and `github.com/foxcpp/go-sieve` dependencies.
   Do not add a second, hand-rolled path for a format a real library already covers, and do not let the
   internal view of data diverge from standard library semantics.
...
"It was easier / faster to write" is never a justification for hand-rolling a parser or formatter for a
standardized format. If a suitable parser is genuinely missing, add the ecosystem dependency or write a real
one with a grammar and tests — never hand-roll ad-hoc scanning.
