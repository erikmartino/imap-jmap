...
func unfoldedVCard(s string) string {
	s = strings.ReplaceAll(s, "\r\n ", "")
	return strings.ReplaceAll(s, "\r\n\t", "")
}

func containsJSON(actual, want any) bool {
	wantMap, isMap := want.(map[string]any)
	if isMap {
		actualMap, ok := actual.(map[string]any)
		if !ok { return false }
		for key, value := range wantMap {
			if actualValue, ok := actualMap[key]; !ok || !containsJSON(actualValue, value) { return false }
		}
		return true
	}
	wantSlice, isSlice := want.([]any)
	if isSlice {
		actualSlice, ok := actual.([]any)
		if !ok || len(actualSlice) != len(wantSlice) { return false }
		for i := range wantSlice { if !containsJSON(actualSlice[i], wantSlice[i]) { return false } }
		return true
	}
	return actual == want
}
...
func TestSmokeToVCard(t *testing.T) {
	cases := []struct {
		...
		{"phone", `{"phones":{"tel0":{"contexts":{"private":true},"features":{"voice":true},"number":"tel:+1-555-555-5555;ext=5555","pref":1}}}`, "TEL;VALUE=uri;PROP-ID=tel0;PREF=1;TYPE=HOME;TYPE=VOICE:tel:+1-555-555-5555;ext=5555"},
		...
		{"org", `{"organizations":{"o1":{"name":"ABC, Inc.","units":[{"name":"North American Division"},{"name":"Marketing"}],"sortAs":"ABC"}}}`, "ORG;PROP-ID=o1;SORT-AS=ABC:ABC\\, Inc.;North American Division;Marketing"},
		...
		{"online_social", `{"onlineServices":{"x2":{"service":"Mastodon","user":"@alice@example2.com","uri":"https://example2.com/@alice","label":"foo"}}}`, "group1.SOCIALPROFILE;PROP-ID=x2;SERVICE-TYPE=Mastodon;USERNAME=@alice@example2.com;VALUE=uri:https://example2.com/@alice"},
		...
		{"langpref", `{"preferredLanguages":{"l1":{"language":"en","contexts":{"work":true},"pref":1}}}`, "LANG;PROP-ID=l1;PREF=1;TYPE=WORK:en"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !strings.Contains(unfoldedVCard(out), tc.want) {
...
func TestSmokeFromVCard(t *testing.T) {
	cases := []struct {
		...
		{"adr", "BEGIN:VCARD\r\nVERSION:4.0\r\nUID:urn:uuid:x\r\nFN:\r\nADR;PROP-ID=a1:;apartment-val;;locality-val;region-val;;;;apartment-val;;;;;;;;;\r\nEND:VCARD\r\n", `"addresses":{"a1":{"components":[{"kind":"locality","value":"locality-val"},{"kind":"region","value":"region-val"},{"kind":"apartment","value":"apartment-val"}]}}`},
