...
type PrincipalsBackend interface {
	...
	PrincipalChanges(ctx context.Context, sinceState string) (created, updated, destroyed []jmapcore.Id, newState string, hasMoreChanges bool)
	GetPrincipals(ctx context.Context, ids []jmapcore.Id) (list []*Principal, notFound []jmapcore.Id, err error)
	...
	UpdatePrincipal(ctx context.Context, id jmapcore.Id, patch map[string]any) (*Principal, error)
	DeletePrincipal(ctx context.Context, id jmapcore.Id) (bool, error)
	QueryPrincipals(ctx context.Context, filter map[string]any, position int, limit *uint64) (ids []jmapcore.Id, total int, err error)
	GetAvailability(ctx context.Context, principalID jmapcore.Id, utcStart, utcEnd string) ([]*AvailabilityWindow, error)
}
