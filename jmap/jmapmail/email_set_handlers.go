...
func HandleEmailSet(backend MailBackend, blobBackend jmapblob.BlobBackend) jmaphandler.MethodHandler {
	return func(ctx context.Context, args map[string]any, clientCallID string) (string, map[string]any) {
		...
		if createMap, ok := args["create"].(map[string]any); ok {
			notCreated = jmaphandler.RunCreateLoop(createMap, creationRefs, func(clientKey string, emData map[string]any) (string, error) {
				...
				if mbMap, ok := emData["mailboxIds"].(map[string]any); ok {
					em.MailboxIDs = make(map[jmapcore.Id]bool)
					for k := range mbMap {
						em.MailboxIDs[jmapcore.Id(k)] = true
					}
				}
				...
				populatePartBlob := func(p *EmailBodyPart) {
					...
					if p.BlobID != nil {
						...
						bID := jmapcore.Id(fmt.Sprintf("blob-%d-%d", time.Now().UnixNano(), len(partData)))
						p.BlobID = &bID
		...
		if destroyList, ok := args["destroy"].([]any); ok {
			for _, rawID := range destroyList {
				if idStr, ok := rawID.(string); ok {
					resolvedID := jmaphandler.ResolveCreationID(idStr, creationRefs)
					if ok, _ := backend.DeleteEmail(ctx, jmapcore.Id(resolvedID)); ok {
						destroyed = append(destroyed, jmapcore.Id(resolvedID))
					} else {
						notDestroyed[string(resolvedID)] = jmapcore.SetError{Type: "notFound", Description: "email not found: " + string(resolvedID)}
					}
				}
			}
		}
		...
		return "Email/set", map[string]any{
			...
			"updated":      jmaphandler.NilIfEmpty(updated),
			"notUpdated":   jmaphandler.NilIfEmpty(notUpdated),
			"notCreated":   jmaphandler.NilIfEmpty(notCreated),
			"destroyed":    jmaphandler.NilIfEmpty(destroyed),
			"notDestroyed": jmaphandler.NilIfEmpty(notDestroyed),
		}
	}
}
...
func validateEmailCreateData(ctx context.Context, accountID string, emData map[string]any, blobBackend jmapblob.BlobBackend) *jmapcore.SetError {
	...
	if _, has := emData["headers"]; has {
		return &jmapcore.SetError{Type: "invalidProperties", Properties: []string{"headers"}}
	}
	...
	if bsRaw, hasBS := emData["bodyStructure"]; hasBS {
		...
		if len(conflictProps) > 0 {
			return &jmapcore.SetError{Type: "invalidProperties", Properties: conflictProps}
		}
	...
	if hbRaw, hasHB := emData["htmlBody"]; hasHB {
		if hbList, ok := hbListRaw(hbRaw); ok {
			if len(hbList) != 1 {
	...
	if len(missingBlobs) > 0 {
		return &jmapcore.SetError{Type: "blobNotFound", NotFound: missingBlobs}
	}
	...
	if bvRaw, hasBV := emData["bodyValues"]; hasBV {
		if bvMap, ok := bvRaw.(map[string]any); ok {
			...
			if len(invalidProps) > 0 {
				return &jmapcore.SetError{Type: "invalidProperties", Properties: invalidProps}
			}
		}
	}
