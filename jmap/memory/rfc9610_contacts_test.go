...
func TestRFC8984_QueryCalendarEvents_Filters(t *testing.T) {
	...
	before, total, _ := b.QueryCalendarEvents(ctx, map[string]any{"before": "2026-08-04T12:00:00Z"}, 0, nil)
	if total != 1 || len(before) != 1 || before[0] != ev1.ID {
		t.Errorf("before filter failed: ids=%v total=%d", before, total)
	}

	after, total, _ := b.QueryCalendarEvents(ctx, map[string]any{"after": "2026-08-04T12:00:00Z"}, 0, nil)
